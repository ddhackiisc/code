...
c	For Native Peptide Parameterization
	subroutine npamppar(iopt)
	include 'mols.par'
...
cs	v1 = 1.000
cs	v2 = 0.020
cs	write(4,100) i,e1,v1,v2,float(k15(i)/2),float(k14(i)-1)
cs	write(4,60) (n15(i,j),j=1,k15(i)),(n14(i,j),j=1,k14(i)-1)
cs	endif
	close(unit=7)
	close(unit=8)
...
c*******************write dihedral angles*************************************
	...
	close(unit=10)
	open(unit=10,file='NPDIHEDS.lib',status='old')
	do j=1,111
...
c******************write hydrogen bond parameters*****************************
	...
	do i=1,natom
	atomna=atname(i)
cs	if(atname(i).eq.' H  '.or.atname(i).eq.' HN ') then
	if(atomna(2:2).eq.'H'.or.atomna(2:3).eq.'HN') then
	do j=i,natom
	atomnb=atname(j)
cs	if(resno(i).lt.resno(j).and.atname(j).eq.' O  ')then
	if(resno(i).lt.resno(j).and.atomnb(2:2).eq.'O')then
	write(4,120) i,j,ihp1,ihp2
        mnhb=mnhb+1
	endif
	enddo
cs	elseif(atname(i).eq.' O  ') then
	elseif(atomna(2:2).eq.'O') then
	do j=i,natom
	atomnb=atname(j)
cs	if(resno(i+1).lt.resno(j).and.atname(j).eq.' H  ')then
	if(resno(i+1).lt.resno(j).and.atomnb(2:2).eq.'H')then
	write(4,120) i,j,ihp1,ihp2
        mnhb=mnhb+1
c	write(31,*)i,j
	endif
	...
	enddo
        write(31,*) 'No. of Hydrogen bond pairs         = ',mnhb
        if(icomment.eq.1)print *,'npamppar-mnhb',mnhb
	close(unit=4)
c***************************************************************
600	return
	end
