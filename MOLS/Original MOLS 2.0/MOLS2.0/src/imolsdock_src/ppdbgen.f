...
c	main program to generate pdb coordinates from amino acid sequences
c	(single letter code(caps or small)-the molecule will be in the
c	extended conformation

c	program pdbgen
	subroutine ppdbgen(seq,fseq)
	include 'mols.par'
...
c------------------------------------------------------------------------
	...
	character fseq(maxatm)*1, scode*1, ch*1, tcode*4, tres(maxatm)*4, 
     ...
     &fatom(maxatm)*4, fatomx(maxatm)*10, xtemp*10,str*80
     	character pat*2, lat*2,patnam*4,
     & presnam*3,lresnam*3,pchid*1,latnam*4,tpres*3, tatnam*4   
...
30      format(a4,4x,i3,a6,a4,a1,i4,4x,3f8.3)
c------------------------------------------------------------------------
...
182	enddo  
        ...
        if(icomment.eq.1)write(*,*) 'The sequence: ',(fseq(i),i=1,lres)
	if(icomment.eq.1)write(*,11)'sequence length: ',lres
...
100	do i=1,lres
	 ...
	 enddo
	if(icomment.eq.1)write(*,*) 'Check your sequence'
	close(unit=1)
200	if(i.eq.1)then
	  iresno=iresno+1
	  do k = 1,natom
...
c
	...
	IF(nat.eq.1)THEN
	open(unit=4,file='pf1.temp',status='unknown')
	ELSE
	open(unit=4,file=pf1,status='unknown')
	ENDIF
	do k=1,iatmno

	  write(4,30)atom(k),iatmno1,mscode(k),tres(k),'X',iresno2(k),
     &    x1(k),y1(k),z1(k)
	  iatmno1=iatmno1+1
	enddo

	close(unit=4)
	if(icomment.eq.1)write(*,11)'No. of pep atoms: ',iatmno1-1
	natom = iatmno1-1

        pbig=(dist(x1(1),y1(1),z1(1),x1(iatmno),y1(iatmno),
     &  z1(iatmno)))/2
