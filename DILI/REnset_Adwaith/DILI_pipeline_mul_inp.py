...
from tensorflow.keras.losses import BinaryCrossentropy, CategoricalCrossentropy, MeanAbsoluteError, MeanSquaredError
...
class Resnet:
    ...
    def build(input_shape, classes, stages, filters, activation, fc_layers, kernel_size=9, stride=1, reg=0.0001, bnEps=2e-5, bnMom=0.9):
        ...
        x = Model(inputs, x, name='Resnet')

        inputs2 = Input(shape=(1444,))
        y = Dense(1000, kernel_regularizer=l2(reg))(inputs2)
        y = Dense(500, kernel_regularizer=l2(reg))(y)
        y = Model(inputs2, y, name='Dense')

        combined = concatenate([x.output, y.output])
        
        z = Dense(classes, activation='softmax',
                  name='last_layer', kernel_regularizer=l2(reg))(combined)
        
        
        # create the model
        model = Model(inputs=[x.input, y.input], outputs=z, name='ensemble')
...
def train_predict(X_train, X_test, y_train, y_test, model_no, lossf, kernel_size, stages, filters, stride, fc_layers, activation, optimizer, reg, batch_size, epochs):
    ...
    def run2(ret_obj):
        ...
        my_model.compile(loss=lossf, optimizer=optimizer, metrics=[
                         'accuracy', AUC(), f1, matthews_correlation, sensitivity, specificity])
        # my_model.summary()
        my_model.fit([X_train[:, :881], X_train[:, 881:]], y_train,
                     batch_size=batch_size, epochs=epochs, verbose=2)
        result = my_model.evaluate([X_test[:, :881], X_test[:, 881:]], y_test, verbose=2)
...
input = data[:, :2325]
label = data[:, 2325:2327]
X_train, X_test, y_train, y_test = sk.train_test_split(
    input, label, test_size=0.1, random_state=42)


stdscale = StandardScaler()
scaled_train = stdscale.fit_transform(X_train[:, 881:])
scaled_test = stdscale.transform(X_test[:, 881:])

X_train = np.concatenate((X_train[:, :881], scaled_train), axis=1)
X_test = np.concatenate((X_test[:, :881], scaled_test), axis=1)



lossf = [BinaryCrossentropy(), CategoricalCrossentropy(
), MeanSquaredError()]
kernel_size = [9]
stages_filters = [[[], [9]]]
stride = [1]
...
reg = [0.001, 0.0001, 0.00001, 0.000001]
optimizer = [Adam(learning_rate=0.0000001), Nadam(learning_rate=0.000001), RMSprop(learning_rate=0.000001), SGD(learning_rate=0.000001)]
batch_size = [47]
epochs = [200]
...
with open('Results_mul.csv', 'w', newline='') as csvfile:
